...
contract CarOwnerList is Utils{
    ...
    function createCarOwner(address _AccidentRecordList,address _BuyRecordList,string memory userName,string memory password,bool gender,string memory phone)
    public {
        address ownerAccount = msg.sender;
        require(isNotRegistered(ownerAccount));
        address newCarOwner = address(new CarOwner(_AccidentRecordList,_BuyRecordList,ownerAccount,userName,password,gender,phone));
    ...
    function isNotRegistered(address account) internal view returns (bool) {
        return creatorOwnerMap[account]== address(0);//if account hasn't create contract,the mapping's default value is 0
    }
...
contract CarOwner is Utils{
    ...
    constructor(address _AccidentRecordList,address _BuyRecordList,address _owner,string memory _userName,string memory _pwd,bool _gender,string memory _phone)  {
        owner = _owner;
    ...
    function addCar(string  memory carNumber,string memory carName,uint8 carAge)  public ownerOnly{
    ...
    function notRepeated(string memory carNumber) internal view returns (bool) {
    ...
    function getCarInfoById(uint id) public view returns (uint,string memory,string memory,uint8,uint) {
...
contract CompanyList is Utils {
    ...
    function createCompany(address _AccidentRecordList,address _BuyRecordList,string memory userName,string memory password,string memory phone,string memory companyNo) public {
        address companyAccount = msg.sender;
        require(isNotRegistered(companyAccount));
        address newCompany = address(new Company(_AccidentRecordList,_BuyRecordList,companyAccount,userName,password,phone,companyNo));
...
contract Company is Utils {
    ...
    constructor(address _AccidentRecordList,address _BuyRecordList,address _owner,string memory _userName,string memory _password,string memory _phone,string  memory _companyNo)  {
        owner = _owner;
        accidentRecordList = _AccidentRecordList;
        buyRecordList = _BuyRecordList;
        userName = _userName;
...
contract PolicerList is Utils{
    ...
    function createPolicer(string memory userName,string  memory password,string memory policerNumber,string memory phone,bool gender) public {
        address policerAccount = msg.sender;
        require(isNotRegistered(policerAccount));
        address newPolicer = address(new Policer(policerAccount,userName,password,policerNumber,phone,gender));
        policerList.push(newPolicer);
        creatorPolicerMap[policerAccount] = newPolicer;
    }

    function isNotRegistered(address account) internal view returns (bool) {
        return creatorPolicerMap[account]== address(0);//if account hasn't create contract,the mapping's default value is 0
    }
    ...
    function getPolicerList() public view returns (address[] memory){
        return policerList;
    }
}
...
contract Policer is Utils {
    ...
    constructor(address _owner,string memory _name,string memory _pwd,string memory _policerNo,string memory _phone,bool _gender)  {
        owner = _owner;
        userName = _name;
        password = stringToBytes32(_pwd);
    ...
    function modifyPolicerInfo(string memory newName,string memory _phone,bool _gender) public ownerOnly {
        userName = newName;
        phone = _phone;
        gender = _gender;
    }
    ...
    function getPolicerInfo() public view returns (string memory,string memory,string memory,bool){
...
contract AccidentRecordList {
    ...
    function addAccidentRecord(address carOwnerListAddr,address carOwner,uint carId,uint time,string memory describe,
        ...
        accidentRecords[nowAccidentRecordId].policer = address(0);
    ...
    function getRecordIdsByOwnerAddr(address carOwnerAddr) public view returns (uint[] memory){
        uint k = 0;
        for(uint i = 0; i < recordList.length; i++) {
    ...
    function getRecordIdsByPolicerAddr(address policerAddr) public view returns (uint[] memory) {
        uint k = 0;
        for(uint i = 0; i < recordList.length; i++) {
    ...
    returns(uint,address,uint,uint,string memory,uint,address,uint,address,uint,uint){
        require(existRecord(recordId));
